...
void Renderer::Initialize()
{
	...
	//shapes[0]->position.x = 0;
	//shapes[0]->rotationSelf = glm::vec3(0.0f);
	//shapes[0]->rotationRate = glm::vec3(0.0f);
	...
	shapes[1] = new Shape(*shapes[0]);


	for (int i = 3; i < 6; i++)
	{
		for (int j = 0; j < vSize / 8; j++)
		{
...
void Renderer::Update(double time) {
	...
	glm::vec3 oldPosition = mCamera->getPosition();// save for collision
	mCamera->Update(time);
